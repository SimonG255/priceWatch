...
const sl: Record<string, string> = {
  ...
  "PriceWatch checks competitor product pages, records price and stock changes, and tells you exactly when it is time to act.": "PriceWatch preverja strani izdelkov konkurentov, beleži spremembe cen in zaloge ter pove, kdaj je čas za ukrepanje.",
  ...
  "COMMON QUESTIONS": "POGOSTA VPRAŠANJA", "Good to know": "Dobro je vedeti", "before you start.": "pred začetkom.", "Which websites can PriceWatch monitor?": "Katere spletne strani lahko PriceWatch spremlja?", "What information do I enter?": "Katere podatke vnesem?", "Can I add many products?": "Ali lahko dodam veliko izdelkov?", "Can PriceWatch bypass blocked pages?": "Ali lahko PriceWatch obide blokirane strani?",
...
const de: Record<string, string> = {
  ...
  "COMMON QUESTIONS": "HÄUFIGE FRAGEN", "Good to know": "Gut zu wissen", "before you start.": "bevor Sie starten.", "Which websites can PriceWatch monitor?": "Welche Websites kann PriceWatch überwachen?", "What information do I enter?": "Welche Angaben muss ich machen?", "Can I add many products?": "Kann ich viele Produkte hinzufügen?", "Can PriceWatch bypass blocked pages?": "Kann PriceWatch gesperrte Seiten umgehen?",
...
export default function LanguageSwitcher() {
  ...
  useEffect(() => {
    ...
    const savedLang = localStorage.getItem("pricewatch-language");
    const lang = queryLang || savedLang;
    if (lang === "sl" || lang === "de") queueMicrotask(() => setLocale(lang));
  }, []);
  useEffect(() => {
    document.documentElement.lang = locale;
    localStorage.setItem("pricewatch-language", locale);
  ...
  function choose(next: Locale) { setLocale(next); localStorage.setItem("pricewatch-language", next); const url = new URL(location.href); if (next === "en") url.searchParams.delete("lang"); else url.searchParams.set("lang", next); history.replaceState({}, "", url); }
