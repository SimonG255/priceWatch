...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const profiles: Array<{
      ...
      [key: string]: unknown;
    }> = await withDatabaseDeadline(db.select().from(customSearchProfiles), "Profile lookup");
    ...
    const [profile] = await withDatabaseDeadline(
      db.update(customSearchProfiles)
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { id } = await params;
    const [profile] = await withDatabaseDeadline(
      getDb().delete(customSearchProfiles).where(eq(customSearchProfiles.id, id)).returning({ id: customSearchProfiles.id }),
      "Profile delete",
    );
    ...
    return Response.json(
      { error: error instanceof Error ? error.message : "Profile could not be deleted." },
      { status: isDatabaseTimeout(error) ? 503 : 400 },
    );
  }
}
